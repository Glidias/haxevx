...
haxe_rtti_XmlParser.prototype = {
	...
	,xoverloads: function(x) {
		...
		while(m.hasNext()) l.add(this.xclassfield(m.next()));
		return l;
	}
	,xpath: function(x) {
		...
		while(c.hasNext()) params.add(this.xtype(c.next()));
	...
	,xtype: function(x) {
		...
		case "a":
			...
			while(f.hasNext()) {
				var f1 = this.xclassfield(f.next(),true);
	...
	,xtypeparams: function(x) {
		var p = new List();
		var c = x.get_elements();
		while(c.hasNext()) p.add(this.xtype(c.next()));
		return p;
	}
	,defplat: function() {
		var l = new List();
...
haxe_xml_Fast.prototype = {
	...
	,get_innerHTML: function() {
		...
		while(x.hasNext()) s_b += Std.string(haxe_xml_Printer.print(x.next()));
		return s_b;
	}
...
haxevx_vuex_core_VxBoot.startStore = function(storeInstance) {
	...
	var sg;
	var c = haxevx_vuex_util_MutatorFactory.getClasses();
	while(c.hasNext()) {
		var c1 = c.next();
		sg = haxevx_vuex_util_ReflectUtil.findSingletonByClassName(Type.getClassName(c1));
		if(sg == null) {
			throw new js__$Boot_HaxeError("Fatal exception occured could not find mutator singleton by class name:" + Type.getClassName(c1));
		}
		haxevx_vuex_util_MutatorFactory.finaliseClass(c1,store);
	}
	var c2 = haxevx_vuex_util_ActionFactory.getClasses();
	while(c2.hasNext()) {
		var c3 = c2.next();
		sg = haxevx_vuex_util_ReflectUtil.findSingletonByClassName(Type.getClassName(c3));
		if(sg == null) {
			throw new js__$Boot_HaxeError("Fatal exception occured could not find action singleton by class name:" + Type.getClassName(c3));
		}
		if(haxevx_vuex_util_ReflectUtil.requiresInjection(null,haxevx_vuex_util_ActionFactory.get_META_INJECTIONS(),c3)) {
			haxevx_vuex_util_RttiUtil.injectFoundSingletonInstances(c3,haxe_rtti_Rtti.getRtti(c3),null,haxevx_vuex_util_ActionFactory.get_META_INJECTIONS());
		}
		haxevx_vuex_util_ActionFactory.finaliseClass(c3,store);
	}
...
haxevx_vuex_core_VxStore.prototype = {
	...
	,_toNative: function() {
		...
		if(this.state == null) {
			...
			var val = storeRTTI.superClass.params.h.item;
			if(this.state == null) {
				if(val[1] == 2) {
	...
	,getModuleTree: function(moduleInstance,moduleNamespace) {
		...
		while(moduleStack.length > 0) {
			...
			if(state == null) {
				...
				var val = haxe_rtti_Rtti.getRtti(cls).superClass.params.h.item;
				if(state == null) {
					if(val[1] == 2) {
			...
			if(fields != null) {
				...
				while(_g3 < _g12.length) {
					...
					var curModule1 = curModule.getters;
					var o1 = insta;
					var name = Type.getClassName(o1 == null ? null : js_Boot.getClass(o1)).split(".").join("_");
					haxevx_vuex_util_GetterFactory.setupGettersFromInstance(insta,curModule1,newModuleNamespace + ((haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE != "" && HxOverrides.substr(name,0,haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE.length) == haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE ? HxOverrides.substr(name,haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE.length,null) : name) + "|"));
				}
			}
...
haxevx_vuex_util_MutatorFactory.finaliseClass = function(cls,store) {
	...
	while(_g < fields.length) {
		...
		++_g;
		var name = Type.getClassName(haxevx_vuex_util_ReflectUtil.getBaseClassForField(cls,f)).split(".").join("_");
		var val = haxevx_vuex_util_MutatorFactory.getMutatorCommit((haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE != "" && HxOverrides.substr(name,0,haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE.length) == haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE ? HxOverrides.substr(name,haxevx_vuex_util_ReflectUtil.PACKAGE_NAMESPACE.length,null) : name) + "|" + f);
		cls.prototype[f] = val;
	}
	cls.prototype["$store"] = store;
};
...
haxevx_vuex_util_ReflectUtil.requiresInjection = function(dynInsMap,metaMap,something) {
	...
	if(dynInsMap != null) {
		var f = dynInsMap.keys();
		while(f.hasNext()) {
			var f1 = f.next();
			if((__map_reserved[f1] != null ? dynInsMap.getReserved(f1) : dynInsMap.h[f1]) && Reflect.field(something,f1) == null) {
				requireInject = true;
				break;
			}
		}
	...
	if(metaMap != null) {
		...
		while(t.hasNext()) {
			var _g = 0;
			var _g1 = Reflect.fields(method(cls,t.next()));
			while(_g < _g1.length) {
				var f2 = _g1[_g];
				++_g;
				if(Reflect.field(something,f2) == null) {
					requireInject = true;
					break;
				}
			}
