 ...
 *  @:optionExplicit flag on abstract class header to require explicitly declaring declaring  all immutable properties manually within abstract class property itself as (get, never) vars during compile to match supplied underlying typeparam (only setPropname() methods will be auto-generated unless the specific field implementation within the abstract is marked with @:getOnly). The underlying typeparam therefore acts somwhat like a Haxe interface  and requires manually declaring those immutable properties within the class as well.
 *  Compile error checking: Finally, ensure all property declarations are Immutable (ie. (get, never)) within abstract class..ie. ensure no other physical properties are included into the abstract class implementation (eg. either through "var" or ":isVar"). All physical properties must be kept outside of the Immutable abstract class itself within the typeparam definition.
 */
