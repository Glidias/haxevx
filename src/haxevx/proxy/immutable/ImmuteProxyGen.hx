...
// The Haxe abstract allows you to handle your own domain-specific methods/accessors/setters and customised access control over the given plain data object which ImmutableJS (or whichever extern you are refering to...) operates on.

/**
 * TODO Roadmap:
 (i figured the below system provides the most flexibility to accomodate different coding  styles for different projects...)
 *  type-param macro immutable prop generation for Typedef vs Class data formats: (Typedef will auto-generate all public immutable properties if not yet already manually defined in abstract class. Class can either auto-generate either private/public immutable properties if not already manually defined in abstract class). The immutable properties are  generated in typical "(get, never)+setPropname" format.
 ...
 *  Compile error checking: All named matching class properties that are also found in Immutable Type Param definition must have matching types and should not have manually defined domain-specific get_method() implementation within the abstract class.
 *  Allow for @:readonly metadata on individual abstract class fields to ensure no accompanying setPropname fields will be auto-generated by the macro from underlying typeparam. However, this metadata doesn't stop you from still being able to manually define your own custom domain-specific setPropname() implementation within the abstract class itself.
 *  @:optionExplicit flag on abstract class header to stop macro property auto-generation completely, and enforce explicitly declaring  all immutable properties manually within abstract class property(get, never) and/or setPropname during compiling to match supplied underlying typeparam. The underlying typeparam therefore acts somwhat like a Haxe interface  and requires manually declaring those immutable properties within the class as well. The difference however is that "setPropname" declarations are optional, and if you do not define it for the given property, the property will effectively be readonly.
 *  Compile error checking: Finally, ensure all property declarations are Immutable (ie. (get, never))..ie. ensure no other physical properties are included into the abstract class implementation (eg. either through "var" or ":isVar").
 */
